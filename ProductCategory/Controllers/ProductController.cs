...
namespace ProductCategory.Controllers
{
    public class ProductController : Controller
    {
            ...
            public ProductController(IConfiguration configuration, Microsoft.AspNetCore.Hosting.IHostingEnvironment env)
            {
                ...
                proCurd = new ProductCurd(this.configuration);
                categoryCurd = new CategoryCurd(this.configuration);
                this.env = env;
            }
            ...
            {
                return View(proCurd.GetProducts());
            }
            ...
            {
                return View(proCurd.GetProductById(id));
            }
            ...
            {
                ViewBag.Category = categoryCurd.GetCategories();
                return View();
            }
            ...
            public ActionResult Create(Product product, IFormFile file)
            {
                try
                {
                    ...
                    var result = proCurd.AddProduct(product);
                    if (result >= 1)
            ...
            {
                var product = proCurd.GetProductById(id);
                ViewBag.Category = categoryCurd.GetCategories();
            ...
            public ActionResult Edit(Product product, IFormFile file)
            {
                try
                {
                    ...
                    var result = proCurd.UpdateProduct(product);
                    if (result >= 1)
                        return RedirectToAction(nameof(Index));
                    else
                    {
            ...
            public ActionResult DeleteConfirm(int id)
            {
                try
                {
                    var emp = proCurd.GetProductById(id);
                    string[] str = emp.Imageurl.Split("/");
                    string str1 = (str[str.Length - 1]);
                    string path = env.WebRootPath + "\\images\\" + str1;
                    System.IO.File.Delete(path);
                    var result = proCurd.DeleteProduct(id);
                    if (result >= 1)
                        return RedirectToAction(nameof(Index));
                    else
                    {
